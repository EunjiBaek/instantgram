...
export default function NewPost({user: {username, image}}: Props) {
    ...
    return (
        <section className='flex flex-col items-center w-full max-w-xl mt-6'>
            ...
            {
                error && <p className="w-full p-4 mb-4 font-bold text-center text-red-600 bg-red-100">
                    {error}
                </p>
            }
            ...
            <form className="flex flex-col w-full mt-2" onSubmit={handleSubmit}>
                ...
                <label className={`w-full h-60 flex flex-col items-center justify-center ${!file && 'border-2 border-sky-500 border-dashed'}`} htmlFor="input-upload" onDragEnter={handleDrag} onDragLeave={handleDrag} onDragOver={handleDragOver} onDrop={handleDrop}>
                    {dragging && (
                        <div className='absolute inset-0 pointer-events-none z-100 bg-sky-500/20' />
                    )}
                    {!file && 
